...
export const GET: APIRoute = async (ctx: APIContext) => {
    try {
        ...
        if (campusErr || areaErr) {
            ...
            return jsonResponse({  ok: false,  message: "Error de servidor o RLS al cargar catálogos.", details: campusErr?.message || areaErr?.message }, 500);
        }

        return jsonResponse({ ok: true, campus, areas }, 200);
