...
async function requireRole(
        ...
        if (!authUser) { return jsonResponse({ ok: false, message: "No autenticado" }, 401);}
        ...
        if (error || !me) { return jsonResponse({ ok: false, message: "Perfil no encontrado" }, 404);}
        if (me.status !== "active") { return jsonResponse({ ok: false, message: "Usuario inactivo" }, 403);}
        if (!roles.includes(me.role as Profile["role"])) { return jsonResponse({ ok: false, message: "Sin permisos" }, 403);}
...
// extrae la ip usando varias entornos conocidas
function getClientIp(request: Request): string {
        const headers = request.headers;

        // lista completa de ips conocidas
        const headerKeys = [
...
export const POST: APIRoute = async (ctx) => {
        ...
        try {
                ...
                if (error || !data?.user) {return jsonResponse({ ok: false, message: error?.message ?? "Error al crear usuario en Auth",},400);}
