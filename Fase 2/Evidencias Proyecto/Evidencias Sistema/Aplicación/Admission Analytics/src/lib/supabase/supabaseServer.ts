...
export function createServerSupabase(ctx: Ctx, opts: Options = {}) {
    ...
    if (!url || !anon) {
        throw new Error("[createServerSupabase] Faltan keys de supabase");
    }
...
export async function getSessionSafe(ctx: Ctx, opts?: Options) {
    ...
    const { data: userData, error: userError, } = await supabase.auth.getUser(); // llama a la API para obtener el usuario actual
    ...
    // retorna supabase, sesion, usuario, perfil y cualquier error
